/*
    TrackerGeneral class derived from Tracker
...
#ifndef TRACKER_GENERAL_H
#define TRACKER_GENERAL_H
...
namespace dvrk {
class TrackerGeneral : public Tracker
{
public:
//    TrackerGeneral(cv::Size _pattern_size = cv::Size(3, 7), cv::Size _roi_size = cv::Size(100, 100));
    ...
    TrackerGeneral(std::vector<cv::Point2f> modelDef);

    bool track(const cv::Mat &cur_image, std::vector<cv::Point2f> &curr_dots, bool found);
