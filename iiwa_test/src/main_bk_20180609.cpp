/********************
*remove subscription functionality from kuka control
*
*
*
********************/
...
Eigen::Matrix<double, 7, 1> destJoints, prevDestJoints;
...
void posCallback_iiwa_currJoints(const sensor_msgs::JointState::ConstPtr& msg){
	...
	tmp.name.push_back("iiwa"+kukaNo+" Joint 1");
	tmp.name.push_back("iiwa"+kukaNo+" Joint 2");
	tmp.name.push_back("iiwa"+kukaNo+" Joint 3");
	tmp.name.push_back("iiwa"+kukaNo+" Joint 4");
	tmp.name.push_back("iiwa"+kukaNo+" Joint 5");
	tmp.name.push_back("iiwa"+kukaNo+" Joint 6");
	tmp.name.push_back("iiwa"+kukaNo+" Joint 7");			
	for (int i = 0; i <7; i++)
...
*/
/*	for (int j = 0; j<7;j++){
		if(!newDest && abs(msg->position[j]-destJoints[j])>0.05){
			cout<<endl;
			cout<<counter<<" destJoints ";
			for (int i = 0; i < 7; i ++){
					destJoints[i] = msg->position[i];
...
////////////////
	...
	ros::Subscriber sub_iiwa_destJoints = nh.subscribe(topicName_destJoints, 1000, posCallback_destJoints);
...
////////////////////////
		...
		//
			...
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 1");
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 2");
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 3");
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 4");
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 5");
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 6");
			msg_currJoints.name.push_back("iiwa"+kukaNo+" Joint 7");			
			for (int i = 0; i <7; i++)
		...
		ofstream outfile1, outfile_received_destJoints, outfile_received_destJoints_int, outfile_kuka_traj;
