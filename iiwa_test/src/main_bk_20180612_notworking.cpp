/********************
*can subscribe from kuka control
*
*
*
********************/
...
////////////////
    ...
    ros::Publisher pub_iiwa_state=nh.advertise<iiwa_test::iiwaState>("iiwa"+kukaNo+"_state", 10, true);
...
////////////////////////
        ...
        //publish iiwa reached, joint state and msrTransform
        iiwa_test::iiwaState tmp_iiwaState;
        tmp_iiwaState.iiwaReached = iiwa_reached;
        tmp_iiwaState.iiwaConnected = iiwa_connected;
        for (int i = 0; i <7; i++){
            tmp_iiwaState.jointState.data.push_back(currJoints[i]);
        }
        for (int j = 0; j <12; j ++){
            tmp_iiwaState.transform.data.push_back(iiwa_msrTransform[j]);
        }
        pub_iiwa_state.publish(tmp_iiwaState);
        rate.sleep();
        //ros::spinOnce();
