...
friUdp::friUdp(int port, const char * remoteHost,const char *localHost) : serverPort(port)
{
	   /* check struct sizes */
...
void friUdp::Init(const char * remoteHost,const char *localHost)
{
...
int friUdp::Recv(tFriMsrData *packet)
{
    if (udpSock >= 0)
    {
        ...
        {
            printf("received something, but wrong size %d (expected %ld)...\n",received, sizeof(tFriMsrData));
            fflush(stdout);
        }
    }
