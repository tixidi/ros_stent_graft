...
/**
	@author G¨¹nter Schreiber <guenter@jacobus>
*/

class friRemote
{
public:
    friRemote(int port = FRI_DEFAULT_SERVER_PORT, const char * hintToRemoteHost=NULL,const char *hinToLocalHost=NULL);
	~friRemote();
	...
	*/
    int doPositionControl(double newJntPosition[LBR_MNJ], bool flagDataExchange=true);
	...
	*/
    int doJntImpedanceControl(const double newJntPosition[LBR_MNJ],
                  const double newJntStiff[LBR_MNJ] = NULL,
                  const double newJntDamp[LBR_MNJ]=NULL,
                  const double newJntAddTorque[LBR_MNJ]=NULL,bool flagDataExchange=true);
	...
	Note: It is possible to change cmd.cmd.cmdFlags in monitor mode only !!
	*/
	int doCartesianImpedanceControl(const double newCartPosition[FRI_CART_FRM_DIM], 
		const double newCartStiff[FRI_CART_VEC]=NULL, 
		const double newCartDamp[FRI_CART_VEC]=NULL, 
		const double newAddTcpFT[FRI_CART_VEC]=NULL,
        const double newJntNullspace[LBR_MNJ]=NULL,  bool flagDataExchange=true);
		/* @} */
	...
	*  Complete desired position inside LBR Kernel is the sum of cmdJntPos and cmdJntPosFriOffset */
	...
	float * getMsrJntTrq() { return msr.data.msrJntTrq; }
		/* @} */
	float *getEstExtTcpFT(){return msr.data.estExtTcpFT;}
