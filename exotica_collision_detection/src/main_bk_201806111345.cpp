...
void write_traj(int iiwaNo, double *iiwa_start_trans_inRob, double *iiwa_end_trans_inRob);
...
double iiwa0_msrTransform[12];
double iiwa1_msrTransform[12];
double iiwa0_currJoints[7];
double iiwa1_currJoints[7];
double iiwa0_desiredEEInRob[12];
double iiwa1_desiredEEInRob[12];
...
int main(int argc,char **argv)
{
	...
	if (argc<3){
		...
		cerr<<"Usage: [mode 0:kuka 1:test] [runRobotIdx 0:iiwa0 1:iiwa1 2:both iiwa]"<<endl;
		return 1;
	}
	...
	while(ros::ok){
		...
		if (mode == 0){
			if (runRobotIdx == 2 && iiwa0_connected && iiwa0_reached && iiwa1_connected && iiwa1_reached && iiwa0_desiredEEInRob_sent && iiwa1_desiredEEInRob_sent){
				write_traj(0, iiwa0_msrTransform, iiwa0_desiredEEInRob);
				write_traj(1, iiwa1_msrTransform, iiwa1_desiredEEInRob);
			...
			if (runRobotIdx == 0 && iiwa0_connected && iiwa0_reached && !iiwa1_connected && !iiwa1_reached && iiwa0_desiredEEInRob_sent && !iiwa1_desiredEEInRob_sent){
				write_traj(0, iiwa0_msrTransform, iiwa0_desiredEEInRob);
				write_traj(1, iiwa1_default_msrTransform, iiwa1_default_desiredEEInRob);
			...
			if (runRobotIdx == 1 && !iiwa0_connected && !iiwa0_reached && iiwa1_connected && iiwa1_reached && !iiwa0_desiredEEInRob_sent && iiwa1_desiredEEInRob_sent){
				write_traj(0, iiwa1_default_msrTransform, iiwa0_default_desiredEEInRob);
				write_traj(1, iiwa1_msrTransform, iiwa1_desiredEEInRob);
		...
		if (mode == 1){
			if (iiwa0_reached && iiwa1_reached && iiwa0_desiredEEInRob_sent && iiwa1_desiredEEInRob_sent){
				//cout<<"<-----------------------11"<<endl;
				write_traj(0, iiwa0_default_msrTransform, iiwa0_desiredEEInRob);
				...
				write_traj(1, iiwa1_default_msrTransform, iiwa1_desiredEEInRob);
			...
			if (!iiwa0_reached && iiwa1_reached && !iiwa0_desiredEEInRob_sent && iiwa1_desiredEEInRob_sent){
				//cout<<"<-----------------------01"<<endl;
				write_traj(0, iiwa0_default_msrTransform, iiwa0_default_desiredEEInRob);
				write_traj(1, iiwa1_default_msrTransform, iiwa1_desiredEEInRob);
				std::copy(std::begin(iiwa1_desiredEEInRob), std::end(iiwa1_desiredEEInRob), std::begin(iiwa1_default_msrTransform));			
				complete_write_traj = true;
				exotica_complete = false;
		...
		if (complete_write_traj){
			...
			while (i < traj_solution.rows()){
				...
				if (iiwa0_reached or iiwa1_reached){
					...
					/*
					for (int j = 0; j <12; j ++){
						double tmp = traj_solution.row(i)[j];
						overall_traj_file<<tmp<<" ";
					}
					overall_traj_file<<endl;*/
